      ...
      datasets: [
        {
          ...
          data: Object.keys(todayData.hours)
            ...
            .map((key) => {
              // @ts-ignore
              return todayData.hours[key].traffic;
            }),
            borderColor: currentBlue,
        },
      ],
    };
    ...
    if (showPreviousDateRange) {
      chartData.datasets.push({
        ...
        data: Object.keys(yesterdayData.hours)
          ...
          .map((key) => {
            // @ts-ignore
            return yesterdayData.hours[key].traffic;
          }),
          borderColor: previousOrange,
      });
    }
    return chartData;
  }
  ...
  if (timePeriod === timePeriods.WEEK) {
    ...
    const chartData = {
      ...
      datasets: [
        {
          label: `Last 7 Days`,
          data: filteredData.map((d) => d.traffic),
          borderColor: currentBlue,
        },
      ],
    };
    ...
    if (showPreviousDateRange) {
      chartData.datasets.push({
        ...
        data: previousData.map((d) => d.traffic),
        borderColor: previousOrange,
      })
    }
